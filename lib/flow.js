    ...
    class Species extends Component {
      ...
      constructor (...args) {
        ...
        this._update = (updatePath, mode) => {
          ...
          let isUpdate
          const isRootImmutable = this.store['__pipe__']._store && TargetComponent.constructor === immutable(Component).constructor
          if (isRootImmutable) {
            isUpdate = true
          } else {
            if (mode === 'batch') updatePath = updatePath.filter(i => i !== null)
            isUpdate = match(updatePath, this._getterPaths, this.store)
          }
          if (isUpdate) {
            this._getterPaths = {}
            this._merged = this._getMergedProps()
